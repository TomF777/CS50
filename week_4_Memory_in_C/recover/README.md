...
The “forensic image” of the card was created, storing its contents, byte after byte, in a file called `card.raw`. 

So that you don’t waste time iterating over millions of 0s unnecessarily, there are only the first few megabytes of the memory card imaged. But you should ultimately find that the image contains 50 JPEGs.
