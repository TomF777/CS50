...
bool print_winner(void)
{
    // more then half of voters needed to win election
...
int find_min(void)
{
    // assume min votes has votes from all voters
    int min_votes = MAX_VOTERS;

    // iterate all candidates
    for (int i = 0; i < candidate_count; i++)
    {
        // check if candidate is not eliminated and he has minimum number of votes
...
bool is_tie(int min)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        // candidate not eliminated and doesn't have min votes
