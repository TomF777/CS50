...
class ControlCommand():
    """Hold current control command status and values"""

    def __init__(self):
        self.run = False
        self.reset = False
        self.z_score_thresh = 2.0
        self.sim_anomaly_level = 0
        self.time_window = 60


class AnomalyDetectZscore:
    def __init__(self, name: str, model_size: int) -> None:
        """
    ...
    def z_score_thresh(self, z_score_threshold: float):
        if z_score_threshold == 0:
            logging.error("Z-score threshold must be above zero")
